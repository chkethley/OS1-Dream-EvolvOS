...
class SelfEvolutionController:
    ...
    def __init__(self, system: Any):
        ...
        self.improvements = []  # History of all improvements
        self.applied_improvements = []  # List of applied improvement IDs
        
        logger.info("Self-evolution controller initialized")
        
    def run_evolution_cycle(self, 
                         max_improvements: int = 1,
                         generation_count: int = 5) -> Dict:
        """
        Run a single evolution cycle.
        
        Args:
            max_improvements: Maximum improvements to apply
            generation_count: Number of improvements to generate
            
        Returns:
            Dictionary with cycle results
        """
        logger.info(f"Starting evolution cycle {self.metrics.cycles_completed + 1}")
        cycle_start = time.time()
        
        # Evaluate current system
        baseline_metrics = self.evaluator.evaluate(self.system)
        logger.info(f"Baseline metrics: {baseline_metrics}")
        
        # Generate potential improvements
        potential_improvements = self.generator.generate_improvements(
            self.system, self.evaluator, generation_count
        )
        
        # Record all potential improvements
        self.improvements.extend(potential_improvements)
        
        # Select best improvements
        selected_improvements = self.selector.select_improvements(
            potential_improvements, max_improvements
        )
        ...
        for improvement in selected_improvements:
            try:
                # In a real system, this would actually modify the system
                # For now, we just simulate applying the improvement
                logger.info(f"Applying improvement: {improvement.description}")
                
                # Mark as applied
                improvement.apply()
                self.applied_improvements.append(improvement.id)
                applied_count += 1
                
            except Exception as e:
                # In case of failure, reject the improvement
                logger.error(f"Failed to apply improvement: {e}")
                logger.error(traceback.format_exc())
                improvement.reject(f"Application failed: {str(e)}")
                self.metrics.record_rejected()
                
        # Record rejected improvements
        rejected_count = len(selected_improvements) - applied_count
        if rejected_count > 0:
            self.metrics.record_rejected(rejected_count)
            
        # Re-evaluate system after improvements
        if applied_count > 0:
            # In a real system, this would measure actual impact
            # For now, we simulate impact based on estimated improvements
            new_metrics = {}
            for metric, value in baseline_metrics.items():
                # Calculate cumulative impact on this metric
                impact = sum(
                    imp.estimated_impact.get(metric, 0)
                    for imp in selected_improvements
                    if imp.applied
                )
                new_metrics[metric] = max(0.0, min(1.0, value + impact))
        else:
            # No improvements applied, metrics unchanged
            new_metrics = baseline_metrics
            
        # Record cycle metrics
        cycle_duration = time.time() - cycle_start
        self.metrics.record_cycle(new_metrics, cycle_duration, applied_count)
        
        logger.info(f"Evolution cycle completed in {cycle_duration:.2f}s")
        logger.info(f"Applied {applied_count} improvements")
        ...
        return {
            ...
            "improvements_generated": len(potential_improvements),
